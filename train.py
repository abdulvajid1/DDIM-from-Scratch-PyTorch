...
def eval(ddim: DDIM, model:UNet, loader: DataLoader, device: str, global_step: int, args: Arguments):
    ...
    sampled_imgs = ddim.sample_image(model, n=args.n_samples, n_steps=args.sampling_steps, eta=0.0)
    save_images(sampled_imgs, path=f'result/{global_step}.jpg')
    mlflow.log_artifact(f'result/{global_step}.jpg')
    
    model.train()
    return avg_loss
    

def train(ddim: DDIM, model: UNet, train_loader: DataLoader, val_loader: DataLoader, sheduler: LambdaLR, optimizer: AdamW, eval_step: int, device:str, epoch: int, args: Arguments, global_step: int):
    model.train()
    progress_bar = tqdm.tqdm(train_loader, desc=f'Epoch: {epoch}')
    if not global_step:
        global_step_now = (epoch * len(train_loader))
    else:
...
def main():
    ...
    with mlflow.start_run(run_id="63343fbbb9b047daa8dbaf5048fd1b4c") as run:
        mlflow.log_params(params=params)

        for epoch in range(args.n_epoch):
            global_step = train(ddim, model, train_loader, val_loader, scheduler, optimizer,  eval_step=args.eval_step, device=device, epoch=epoch, args=args, global_step=global_step)
